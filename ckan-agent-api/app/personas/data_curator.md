...
- Findable: persistent identifiers referenced, rich/searchable metadata present, tags/categories populated, title is descriptive and discovery-friendly.
  **TITLE TYPE CHECK (blocking):** A catalog-quality title must include TYPE (what kind of data), PLACE (where), and TIME (when) when those elements are derivable from the resource list, file inventory, or notes. When the resource list or inventory clearly indicates the data type — e.g. GTiff/TIF → `orthophoto`; LAZ/LAS → `point cloud`; LAZ+TIF together → `orthophoto and SfM point cloud`; SHP → `shapefile`; CSV → `tabular data` — a title that omits the data type entirely is a **BLOCKING** findability failure. Return `verdict: revise` with a question on field `title` and `requires_human: false` instructing the author to prepend the data type (the author can derive it from the resource list). A title that is only a place name, a project name, or a vague phrase with no type element fails this check. Missing PLACE or TIME when derivable is also blocking. Only treat title quality as non-blocking when type/place/time genuinely cannot be determined from any source.
