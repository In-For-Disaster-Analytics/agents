...
# How much notebook code to surface to the author (bbox/date/CRS values live in code cells).
NOTEBOOK_CODE_PREVIEW_CHARS = 6000
...
def profile_json(path: Path, *, max_sample_chars: int) -> dict[str, Any]:
    ...
    if isinstance(data, dict) and data.get("nbformat") and isinstance(data.get("cells"), list):
        return _notebook_profile(data)
...
def _notebook_profile(data: dict[str, Any]) -> dict[str, Any]:
    """Structured summary of a Jupyter notebook: cell counts, markdown headings, imports."""
    ...
    for cell in cells:
        ...
        if cell.get("cell_type") == "code":
            code_src.append(text)
        elif cell.get("cell_type") == "markdown":
            md_src.append(text)
    code_text = "\n".join(code_src)
    md_text = "\n".join(md_src)
    imports = sorted(
        {m.group(1).split(".")[0] for m in re.finditer(r"(?m)^\s*(?:from|import)\s+([A-Za-z_][\w.]*)", code_text)}
    )[:30]
    headings = [m.group(1).strip()[:160] for m in re.finditer(r"(?m)^\s*#{1,6}\s+(.+)$", md_text)][:15]
    return {
        "notebook": {
            "cell_count": len(cells),
            "code_cell_count": len(code_src),
            "markdown_cell_count": len(md_src),
            "language": ((data.get("metadata") or {}).get("kernelspec") or {}).get("language"),
            "python_imports": imports,
            "markdown_headings": headings,
            "first_markdown": (md_src[0][:500] if md_src else ""),
            # The actual code — so the author can read values that live in the code itself
            # (bounding boxes, date ranges, CRS, variable definitions) instead of re-asking.
            "code_preview": code_text[:NOTEBOOK_CODE_PREVIEW_CHARS],
            "code_truncated": len(code_text) > NOTEBOOK_CODE_PREVIEW_CHARS,
        }
    }


def looks_like_notebook(path: Path) -> bool:
    """Cheap check: a JSON file whose root has nbformat + a cells array."""
    try:
        with path.open("rb") as handle:
            head = handle.read(4096)
    except OSError:
        return False
    return b'"nbformat"' in head and b'"cells"' in head


def read_notebook_source(path: Path, *, max_chars: int) -> dict[str, Any]:
    """Concatenated markdown + code cell sources of a notebook, in order — readable plain
    text rather than the raw escaped notebook JSON, so the author can scan the actual code."""
    data = json.loads(path.read_text(encoding="utf-8", errors="replace"))
    parts: list[str] = []
    for cell in data.get("cells") or []:
        ...
        parts.append(f"# --- {kind} cell ---\n{text}" if kind == "markdown" else text)
    joined = "\n\n".join(parts)
    ...
    return {
        ...
        "notebook": True,
        "truncated": truncated,
        "characters_returned": min(len(joined), max_chars),
    }
